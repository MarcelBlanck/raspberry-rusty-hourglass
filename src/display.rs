...
//https://docs.rs/crate/rppal/0.11.3/source/examples/spi_25aa1024.rs
...
impl<T: SpiInterface, U: PinInterface> Display<T, U> {
    fn new(spi: T, reset_pin: U, dc_pin: U) -> Display<T, U> {
	...
	fn fill_with_black(&mut self) {
		self.buffer = [0u8; 512];
	}

	fn fill_with_white(&mut self) {
		self.buffer = [255u8; 512];
	}

	fn fill_with_pixmap(&mut self, pixmap: &Pixmap) {

	}

	fn write_ascii_text(&mut self, text: AsciiText) {

	}

	fn get_pixel_color(&self, point: &Point) -> Result<Color, &'static str> {
	...
	fn set_pixel_color(&mut self, point: &Point, color: &Color) {
		let byte = point.y * 4 + (point.x / 8);
		if byte >= 0 && byte < self.buffer.len() as isize {
			let byte_checked: usize = byte as usize;
			let bit_mask = 1u8 << (point.x % 8);
	...
	fn toggle_pixel(&mut self, point: &Point) {
	...
	fn draw_box(&mut self, bottom_left: &Point, top_right: &Point, border_color: &Color, fill_color: &Color) {
		...
		for y in (bottom_left.y + 1)..top_right.y {
			...
			for x in (bottom_left.x + 1)..top_right.x {
				self.set_pixel_color(&Point{x: x, y: y}, fill_color);
			} 
		}
	}
}
